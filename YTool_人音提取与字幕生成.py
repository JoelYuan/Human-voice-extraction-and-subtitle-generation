...
class AudioTranscriptionTool:
    ...
    def create_gui(self):
        ...
        # 标题标签
        title_label = ttk.Label(main_frame, text="人音提取与字幕生成工具", font=("Segoe UI", 14, "bold"))
        ...
        for fmt in self.supported_subtitle_formats:
            ttk.Radiobutton(format_frame_inner, text=fmt.upper(), variable=self.selected_subtitle_format, value=fmt).pack(side=tk.LEFT, padx=15)
    ...
    def load_vad_model(self):
        ...
        try:
            ...
            self.vad_model = ort.InferenceSession(vad_model_path)
            return True
        except Exception as e:
            messagebox.showerror("错误", f"加载VAD模型失败: {str(e)}")
            return False
    ...
    def load_whisper_model(self):
        ...
        try:
            ...
            if not os.path.exists(model_path):
                raise FileNotFoundError(f"Whisper模型文件夹不存在: {model_path}")
            # 使用CPU运行，不量化
        ...
        try:
            ...
            with open(subtitle_file_path, "w", encoding="utf-8") as subtitle_file:
                for i, seg_info in enumerate(segment_files):
                    ...
                    self.progress_var.set(progress)
                    self.status_var.set(f"正在转录语音段 {i+1}/{total_segments}")
                    self.root.update()
    ...
    def start_processing(self):
        ...
        try:
            ...
            self.status_var.set("正在加载Whisper模型...")
            self.root.update()
